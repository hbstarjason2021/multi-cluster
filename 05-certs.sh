...
openssl x509 -req -in ./certs/cert.csr -CA ./certs/rootCA.crt -CAkey ./certs/rootCA.key -CAcreateserial -out ./certs/cert.pem -days 365 -sha256
cp ./certs/rootCA.crt ./certs/cacerts.pem


#####
#####  for i in $(ls *.pem |grep -v key); do echo "===== $i ====="; openssl x509 -in $i -text -noout | grep -A 3 'Validity' ; done
